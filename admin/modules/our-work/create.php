<?php
/**
 * Our Work Management - Upload New Image
 */
...
require_permission('our_work_create');

$page_title = 'Upload Our Work Image';
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!validate_csrf_token($csrf_token)) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) {
      ...
      $image = $uploader->upload($_FILES['image'], 'our_work');
      if (!$image) {
    ...
    if (empty($errors)) {
      try {
        $stmt = $pdo->prepare("INSERT INTO gallery (title, image, category, display_section, status, uploaded_by, created_at) VALUES (?, ?, ?, 'our_work', ?, ?, NOW())");
        $stmt->execute([$title, $image, $category, $status, $_SESSION['admin_id']]);

        $image_id = $pdo->lastInsertId();
        log_activity('create', 'our_work', $image_id, "Uploaded Our Work image: $title");

        set_flash('success', 'Our Work image uploaded successfully!');
...
<div class="page-header">
  <h1><i class="bi bi-cloud-upload me-2"></i>Upload Our Work Image</h1>
</div>
